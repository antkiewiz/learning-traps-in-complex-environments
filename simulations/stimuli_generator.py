...
def generate_abstract_stims(ndim):
    ...
    #stimset =  [[(num/place)%2 for place in powers] for num in range(2 ** (ndim+1))] # 6apr create larger stimset
    return stimset
...
def stimuli_generator(condition, counterbalance, numblocks, numtestblocks, ndim=4,
    ...
    #stimtypes = [0 for _ in range(nstim/4)] + [1 for _ in range(nstim-nstim/4)]  # orig
    stimtypes = [0 for _ in range(int(nstim/4))] + [1 for _ in range(int(nstim-nstim/4))]  # add int() 6apr
    # stimulus payoffs
    ...
    indiv_values = list(range(nstim))  # 6apr
    ...
    for b in range(numblocks):
        ...
        for s in stims:
            ...
            physdimensionvals = stimset[ int(phys_stimnums[s]) ]  # add int()  6apr
